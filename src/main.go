...
func main() {
	_ = godotenv.Load()
        ...
        nodeNameMsg := ""
        if nodeName != "" {
		nodeNameMsg = fmt.Sprintf("%s | ", nodeName)
	}
	...
	for {
		...
		if err != nil {
			...
			if lastStatus != "down" {
				sendTelegramMessage(fmt.Sprintf("🚨 %s Monad RPC is DOWN! Unable to connect to port.", nodeNameMsg))
		...
		if lastStatus == "down" {
			sendTelegramMessage(fmt.Sprintf("✅ %s Monad RPC is back UP! Current height: %d", nodeNameMsg, currentHeight))
			writeStringToFile(lastStatusFile, "ok")
		}

		if lastHeight == currentHeight && lastStatus != "stuck" {
			msg := fmt.Sprintf("⚠️ %s Monad node stuck at height: %d", nodeNameMsg, currentHeight)
			sendTelegramMessage(msg)
			writeStringToFile(lastStatusFile, "stuck")

		} else if lastHeight != -1 && lastHeight != currentHeight && lastStatus == "stuck" {
			msg := fmt.Sprintf("✅ %s Monad node syncing resumed! Current height: %d", nodeNameMsg, currentHeight)
			sendTelegramMessage(msg)
			writeStringToFile(lastStatusFile, "ok")
		}
